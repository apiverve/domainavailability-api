...
class DomainavailabilityValidationException implements Exception {
  final List<String> errors;

  DomainavailabilityValidationException(this.errors);

  @override
  String toString() => 'DomainavailabilityValidationException: ${errors.join("; ")}';
}
...
///
/// For more information, visit: https://domainavailability.apiverve.com?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [domain] (required) - The domain to check if it is available
class DomainavailabilityClient {
  ...
  DomainavailabilityClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/domainavailability',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [DomainavailabilityValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw DomainavailabilityValidationException(errors);
    }
  }

  /// Execute a request to the Domain Availability API.
  ///
  /// Parameters are validated before sending the request.
  Future<DomainavailabilityResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw DomainavailabilityException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return DomainavailabilityResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw DomainavailabilityException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw DomainavailabilityException('Resource not found');
      } else {
        throw DomainavailabilityException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is DomainavailabilityException) rethrow;
      throw DomainavailabilityException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Domain Availability API client.
class DomainavailabilityException implements Exception {
  final String message;

  DomainavailabilityException(this.message);

  @override
  String toString() => 'DomainavailabilityException: $message';
}
